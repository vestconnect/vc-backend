...
export default class ProductsTagsNfcController {
  public async index(request: Request, response: Response): Promise<Response> {
    const selectProductTagNfcServices = container.resolve(
      SelectProductTagNfcServices
    );
    const product_id = request.params.id;
    const { page } = request.query;

    const productTag = await selectProductTagNfcServices.execute({
      product_id,
      page: Number(page),
    });
  ...
  public async execute(
    ...
    const createProductTagNfcServices = container.resolve(
      CreateProductTagNfcServices
    );
    const product_id = request.params.id;
  ...
  public async inactive(
    ...
    const updateActiveProductsTagsNfcServices = container.resolve(
      UpdateActiveProductsTagsNfcServices
    );
